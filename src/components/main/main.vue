<template>
  <el-container
    ...
    :class="collapsed ? 'aside-min' : 'aside-max'"
  >
    <!-- 左侧 -->
    <left-aside
      :collapsed="collapsed"
...
export default {
  computed: {
    ...
    ...mapGetters(["menuList", "avatarImage"]),
    cacheList() {
      const list = [
        "ParentView",
  ...
  methods: {
    ...
    closeAllTagEvent() {
      let res = this.tagNavList.filter(
        item => item.name === config.ROUTER.HOME_NAME
      );
    ...
    closeOtherTagEvent() {
      let res = this.tagNavList.filter(
        item =>
          routeEqual(item, this.$route) || item.name === config.ROUTER.HOME_NAME
      );
      this.setTagNavList(res);
    },
    updateCollapsed() {
